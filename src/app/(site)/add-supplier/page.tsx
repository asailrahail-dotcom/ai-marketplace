...
import { createSupplier } from '@/app/actions/listings';

export default async function AddSupplierPage({ searchParams }: { searchParams: { error?: string } }) {
  ...
  return (
    <div className="container-content section max-w-xl">
      <h1 className="h2 mb-8">أضف كمورد</h1>
      ...
      <form action={createSupplier} className="space-y-4">
        <div>
          <label className="text-sm font-medium block mb-1.5">اسم المؤسسة</label>
          <input name="name" required className="input" />
        </div>
        <div>
          <label className="text-sm font-medium block mb-1.5">التخصص</label>
          <input name="specialty" required className="input" placeholder="مثال: توريد أثاث تجاري" />
        </div>
        <div>
        ...
        <div>
          <label className="text-sm font-medium block mb-1.5">رابط الشعار (اختياري)</label>
          <input name="logo" type="url" className="input" placeholder="https://…" />
        </div>
